...
const PASSWORD_COMBINATION_REGEX = {
  regex: /^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-zA-Z])(?=.*[0-9]))|((?=.*[0-9])(?=.*[$`~!@$!%*#^?&\\(\\)\-_=+]))|((?=.*[a-z])(?=.*[A-Z])|(?=.*[a-zA-Z])(?=.*[$`~!@$!%*#^?&\\(\\)\-_=+]))).{10,}/,
  isMatch: true,
}
const PASSWORD_SEQUENCE_REGEX = {
  regex: /012|123|234|345|456|567|678|789|987|876|765|654|543|432|321|210|([0-9])\1\1/gm,
  isMatch: false,
}
const BIRTHDAY_REGEX = {
  regex: /^(19|20)\d{2}.(0[1-9]|1[0-2]).(0[1-9]|1[0-9]|2[0-9]|3[0-1])$/,
  isMatch: true,
}

const testRegex = (regexList) => (string) => {
  const isPassedAll = regexList.reduce((result, regexObject) => {
    const {regex, isMatch} = regexObject;
    const isOk = regex.test(string);
